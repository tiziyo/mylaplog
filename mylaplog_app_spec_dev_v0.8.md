...
### (5) 🛑 정밀 GPS 피트레인 타이머 (`GPS Pit Lane Timer`)
- **듀얼 게이트 지오펜스**: 피트 진입선(Pit Entry) 및 피트 출구선(Pit Exit) 통과 자동 감지
- **피트레인 속도 감시**: 규정 속도(40/50/60 km/h) 실시간 감시, 초과 시 시각적 RED 점멸 + 오디오 사이렌 경고
- **정차 시간 & 릴리즈 카운트다운**: 피트 박스 정차(속도 < 2.5 km/h) 감지, 의무 피트(5분 기본 / 60s) / 정차(45s / 30s) 규정시간 대비 3-2-1 비프 카운트다운 및 녹색 GO 릴리즈 차임
- **규정 시간 임박 알람 설정 & 고시인성 경보**: 50초/40초/30초(기본)/20초 전 또는 OFF 설정 지원. 잔여 시간 도달 시 앰버 펄스 애니메이션, 대형 상단 배너, 4-하모닉 알람 차임 및 진동(Vibration) 동시 발동으로 드라이버 시인성 극대화
- **피트스탑 이력 및 델타 분석**: 총 피트 통과시간, 순수 정차시간, 피트 최고속도, 규정 충족 델타(+/-) 실시간 분석 (피트 초기화 시 이전 기록 전체 초기화)
- **가상 피트스탑 시뮬레이션**: 실내에서도 진입 대기 -> 진입 -> 정차(임박 알람) -> 릴리즈 -> 출구 풀 사이클 테스트 지원
