...
- **내 소속 팀 전체 카드 목록 (최대 10개)**:
  ...
  - 각 팀 카드별 팀명, 역할(CHIEF/DRIVER/MECHANIC), 홈 서킷, 초대 코드 복사, [팀 선택/조회], [팀 삭제/탈퇴] 액션 제공
...
- **신규 세션 등록 모달 (`Add Modal`)**:
  - 서킷 선택 (인제, 영암, 용인, 태백)
  - 개러지 등록 머신 선택 연동
  - 4륜 PSI, 댐퍼 클릭수, 캠버, 엔지니어링 메모 저장
- **세션 & 셋업 정보 수정 (`Edit Modal`)**: 주행 일자, 세션 번호, 베스트 랩타임, 4륜 공기압 및 댐퍼/캠버 셋업 실시간 수정
- **세션 로그 삭제**: 불필요한 주행 세션 및 연결된 셋업 데이터 삭제

### (5) 🏎️ 개러지 머신 관리 (`Garage`)
- **머신 스펙 카드**: 제조사, 모델, 연식, 엔진 마력(hp), 장착 타이어 모델, 타이어 규격, 하체 서스펜션 사양
- **공개 범위 태그**: `TEAM` (팀원 공유), `PUBLIC` (전체 공개), `PRIVATE` (나만 보기)
- **신규 머신 등록 모달 (`Modal`)**: 제조사, 모델명, 마력, 타이어 규격 등 입력 지원
- **머신 스펙 정보 수정 (`Edit Modal`)**: 등록된 머신의 출력, 타이어, 하체 셋업, 공개 범위를 실시간으로 수정 및 저장
- **머신 삭제**: 불필요한 개러지 차량 삭제 지원
